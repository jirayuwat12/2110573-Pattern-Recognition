...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0, dropout=0.0):
    ...
    #############################################################################
    droped_idx = np.random.rand(W1.shape[1]) > (1-dropout)
    W1_droped = W1.copy()
    W1_droped[:, droped_idx] = 0
    W2_droped = W2.copy()
    W2_droped[droped_idx, :] = 0

    o_layer1 = np.maximum(np.dot(X, W1_droped) + b1, 0)
    o_layer2 = np.dot(o_layer1, W2_droped) + b2
    ...
    # classifier loss.                                                          #
    #############################################################################
    soft_max_scores = np.exp(scores) / np.sum(np.exp(scores), axis=1, keepdims=True)

    loss = -np.sum(np.log(soft_max_scores[range(N), y])) / N
    loss +=  reg * (np.sum(W1_droped * W1_droped) + np.sum(W2_droped * W2_droped)) /2
    #############################################################################
    #                              END OF TODO#2                                #
    #############################################################################
    ...
    #############################################################################
    d_scores = soft_max_scores
    d_scores[range(N), y] -= 1
    d_scores /= N
    grads['W2'] = np.dot(o_layer1.T, d_scores) + reg * W2_droped
    grads['b2'] = np.sum(d_scores, axis=0)

    d_layer1 = np.dot(d_scores, W2.T)
    d_layer1[o_layer1 <= 0] = 0
    grads['W1'] = np.dot(X.T, d_layer1) + reg * W1_droped
    grads['b1'] = np.sum(d_layer1, axis=0)
    #############################################################################
    #                              END OF TODO#3                                #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
            ...
            reg=5e-6, num_iters=100, dropout=0.0,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      idx = np.random.choice(num_train, batch_size, replace=batch_size > num_train)
      X_batch = X[idx]
      y_batch = y[idx]
      #########################################################################
      #                             END OF YOUR TODO#4                        #
